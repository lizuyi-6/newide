...
const nlsSingleSelectionRange = localize('singleSelectionRange', "行 {0}, 列 {1} ({2} 已选)");
const nlsSingleSelection = localize('singleSelection', "行 {0}, 列 {1}");
const nlsMultiSelectionRange = localize('multiSelectionRange', "{0} 个选择 ({1} 个字符已选)");
...
class EditorStatus extends Disposable {
	...
	private async showIndentationPicker(): Promise<unknown> {
		...
		if (this.editorService.activeEditor?.isReadonly()) {
			return this.quickInputService.pick([{ label: localize('noWritableCodeEditor', "当前代码编辑器是只读的。") }]);
		}
	...
	private updateTabFocusModeElement(visible: boolean): void {
		if (visible) {
			if (!this.tabFocusModeElement.value) {
				const text = localize('tabFocusModeEnabled', "Tab 移动焦点");
				this.tabFocusModeElement.value = this.statusbarService.addEntry({
					name: localize('status.editor.tabFocusMode', "无障碍模式"),
					text,
					ariaLabel: text,
					tooltip: localize('disableTabMode', "禁用无障碍模式"),
	...
	private updateEOLElement(text: string | undefined): void {
		...
		const props: IStatusbarEntry = {
			name: localize('status.editor.eol', "编辑器行尾"),
			text,
			ariaLabel: text,
