...
MenuRegistry.appendMenuItem(MenuId.EditorContext, {
	...
	title: nls.localize('peek.submenu', "查看"),
	group: 'navigation',
	order: 100
...
export class DefinitionAction extends SymbolNavigationAction {
	...
	protected async _getLocationModel(languageFeaturesService: ILanguageFeaturesService, model: ITextModel, position: corePosition.Position, token: CancellationToken): Promise<ReferencesModel> {
		return new ReferencesModel(await getDefinitionsAtPosition(languageFeaturesService.definitionProvider, model, position, false, token), nls.localize('def.title', '定义'));
	}

	protected _getNoResultFoundMessage(info: IWordAtPosition | null): string {
		return info && info.word
			? nls.localize('noResultWord', "未找到'{0}'的定义", info.word)
			: nls.localize('generic.noResults', "未找到定义");
	}
...
registerAction2(class GoToDefinitionAction extends DefinitionAction {
	...
	constructor() {
		super({
			...
			title: {
				...nls.localize2('actions.goToDecl.label', "转到定义"),
				mnemonicTitle: nls.localize({ key: 'miGotoDefinition', comment: ['&& denotes a mnemonic'] }, "转到定义(&&D)"),
			},
...
registerAction2(class OpenDefinitionToSideAction extends DefinitionAction {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.goToDeclToSide.label', "在侧边打开定义"),
...
registerAction2(class PeekDefinitionAction extends DefinitionAction {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.previewDecl.label', "查看定义"),
			precondition: ContextKeyExpr.and(
				EditorContextKeys.hasDefinitionProvider,
...
class DeclarationAction extends SymbolNavigationAction {
	...
	protected async _getLocationModel(languageFeaturesService: ILanguageFeaturesService, model: ITextModel, position: corePosition.Position, token: CancellationToken): Promise<ReferencesModel> {
		return new ReferencesModel(await getDeclarationsAtPosition(languageFeaturesService.declarationProvider, model, position, false, token), nls.localize('decl.title', '声明'));
	}

	protected _getNoResultFoundMessage(info: IWordAtPosition | null): string {
		return info && info.word
			? nls.localize('decl.noResultWord', "未找到'{0}'的声明", info.word)
			: nls.localize('decl.generic.noResults', "未找到声明");
	}
...
registerAction2(class GoToDeclarationAction extends DeclarationAction {
	...
	constructor() {
		super({
			...
			title: {
				...nls.localize2('actions.goToDeclaration.label', "转到声明"),
				mnemonicTitle: nls.localize({ key: 'miGotoDeclaration', comment: ['&& denotes a mnemonic'] }, "转到声明(&&D)"),
			},
			precondition: ContextKeyExpr.and(
...
registerAction2(class PeekDeclarationAction extends DeclarationAction {
	constructor() {
		super({
			...
			title: nls.localize2('actions.peekDecl.label', "查看声明"),
			precondition: ContextKeyExpr.and(
				EditorContextKeys.hasDeclarationProvider,
				PeekContext.notInPeekEditor,
				EditorContextKeys.isInEmbeddedEditor.toNegated()
...
class TypeDefinitionAction extends SymbolNavigationAction {
	...
	protected async _getLocationModel(languageFeaturesService: ILanguageFeaturesService, model: ITextModel, position: corePosition.Position, token: CancellationToken): Promise<ReferencesModel> {
		return new ReferencesModel(await getTypeDefinitionsAtPosition(languageFeaturesService.typeDefinitionProvider, model, position, false, token), nls.localize('typedef.title', '类型定义'));
	}

	protected _getNoResultFoundMessage(info: IWordAtPosition | null): string {
		return info && info.word
			? nls.localize('goToTypeDefinition.noResultWord', "未找到'{0}'的类型定义", info.word)
			: nls.localize('goToTypeDefinition.generic.noResults', "未找到类型定义");
	}
...
registerAction2(class GoToTypeDefinitionAction extends TypeDefinitionAction {
	...
	constructor() {
		super({
			...
			title: {
				...nls.localize2('actions.goToTypeDefinition.label', "转到类型定义"),
				mnemonicTitle: nls.localize({ key: 'miGotoTypeDefinition', comment: ['&& denotes a mnemonic'] }, "转到类型定义(&&T)"),
			},
...
registerAction2(class PeekTypeDefinitionAction extends TypeDefinitionAction {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.peekTypeDefinition.label', "查看类型定义"),
			precondition: ContextKeyExpr.and(
...
class ImplementationAction extends SymbolNavigationAction {
	...
	protected async _getLocationModel(languageFeaturesService: ILanguageFeaturesService, model: ITextModel, position: corePosition.Position, token: CancellationToken): Promise<ReferencesModel> {
		return new ReferencesModel(await getImplementationsAtPosition(languageFeaturesService.implementationProvider, model, position, false, token), nls.localize('impl.title', '实现'));
	}

	protected _getNoResultFoundMessage(info: IWordAtPosition | null): string {
		return info && info.word
			? nls.localize('goToImplementation.noResultWord', "未找到'{0}'的实现", info.word)
			: nls.localize('goToImplementation.generic.noResults', "未找到实现");
	}
...
registerAction2(class GoToImplementationAction extends ImplementationAction {
	...
	constructor() {
		super({
			...
			title: {
				...nls.localize2('actions.goToImplementation.label', "转到实现"),
				mnemonicTitle: nls.localize({ key: 'miGotoImplementation', comment: ['&& denotes a mnemonic'] }, "转到实现(&&I)"),
			},
...
registerAction2(class PeekImplementationAction extends ImplementationAction {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.peekImplementation.label', "查看实现"),
			precondition: ContextKeyExpr.and(
...
abstract class ReferencesAction extends SymbolNavigationAction {
	...
	protected _getNoResultFoundMessage(info: IWordAtPosition | null): string {
		return info
			? nls.localize('references.no', "未找到'{0}'的引用", info.word)
			: nls.localize('references.noGeneric', "未找到引用");
	}
...
registerAction2(class GoToReferencesAction extends ReferencesAction {
	...
	constructor() {
		super({
			...
			title: {
				...nls.localize2('goToReferences.label', "转到引用"),
				mnemonicTitle: nls.localize({ key: 'miGotoReference', comment: ['&& denotes a mnemonic'] }, "转到引用(&&R)"),
			},
			precondition: ContextKeyExpr.and(
	...
	protected async _getLocationModel(languageFeaturesService: ILanguageFeaturesService, model: ITextModel, position: corePosition.Position, token: CancellationToken): Promise<ReferencesModel> {
		return new ReferencesModel(await getReferencesAtPosition(languageFeaturesService.referenceProvider, model, position, true, false, token), nls.localize('ref.title', '引用'));
	}
});
...
registerAction2(class PeekReferencesAction extends ReferencesAction {
	...
	constructor() {
		super({
			...
			title: nls.localize2('references.action.label', "查看引用"),
			precondition: ContextKeyExpr.and(
				EditorContextKeys.hasReferenceProvider,
				PeekContext.notInPeekEditor,
				EditorContextKeys.isInEmbeddedEditor.toNegated()
	...
	protected async _getLocationModel(languageFeaturesService: ILanguageFeaturesService, model: ITextModel, position: corePosition.Position, token: CancellationToken): Promise<ReferencesModel> {
		return new ReferencesModel(await getReferencesAtPosition(languageFeaturesService.referenceProvider, model, position, false, false, token), nls.localize('ref.title', '引用'));
	}
});
...
class GenericGoToLocationAction extends SymbolNavigationAction {
	...
	constructor(
		...
		super(config, {
			...
			title: nls.localize2('label.generic', "转到任意符号"),
			precondition: ContextKeyExpr.and(
				PeekContext.notInPeekEditor,
				EditorContextKeys.isInEmbeddedEditor.toNegated()
			),
	...
	protected async _getLocationModel(languageFeaturesService: ILanguageFeaturesService, _model: ITextModel, _position: corePosition.Position, _token: CancellationToken): Promise<ReferencesModel | undefined> {
		return new ReferencesModel(this._references, nls.localize('generic.title', '位置'));
	}

	protected _getNoResultFoundMessage(info: IWordAtPosition | null): string {
		return info && nls.localize('generic.noResult', "未找到'{0}'的结果", info.word) || '';
	}
