...
export class NextMarkerAction extends MarkerNavigationAction {
	...
	static LABEL = nls.localize2('markerAction.next.label', "转到下一个问题 (错误、警告、信息)");
	constructor() {
		super(true, false, {
...
export class PrevMarkerAction extends MarkerNavigationAction {
	...
	static LABEL = nls.localize2('markerAction.previous.label', "转到上一个问题 (错误、警告、信息)");
	constructor() {
		super(false, false, {
...
export class NextMarkerInFilesAction extends MarkerNavigationAction {
	...
	constructor() {
		super(true, true, {
			...
			label: nls.localize2('markerAction.nextInFiles.label', "转到文件中的下一个问题 (错误、警告、信息)"),
			precondition: undefined,
			kbOpts: {
				kbExpr: EditorContextKeys.focus,
				primary: KeyCode.F8,
			...
			menuOpts: {
				...
				title: nls.localize({ key: 'miGotoNextProblem', comment: ['&& denotes a mnemonic'] }, "下一个问题(&&P)"),
				group: '6_problem_nav',
				order: 1
			}
		});
...
export class PrevMarkerInFilesAction extends MarkerNavigationAction {
	...
	constructor() {
		super(false, true, {
			...
			label: nls.localize2('markerAction.previousInFiles.label', "转到文件中的上一个问题 (错误、警告、信息)"),
			precondition: undefined,
			kbOpts: {
				kbExpr: EditorContextKeys.focus,
			...
			menuOpts: {
				menuId: MenuId.MenubarGoMenu,
				title: nls.localize({ key: 'miGotoPreviousProblem', comment: ['&& denotes a mnemonic'] }, "上一个问题(&&P)"),
				group: '6_problem_nav',
				order: 2
			}
		});
