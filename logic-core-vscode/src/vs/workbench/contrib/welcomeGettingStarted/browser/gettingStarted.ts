...
export class GettingStartedPage extends EditorPane {
		...
		this.container = $('.gettingStartedContainer',
			{
				role: 'document',
				tabindex: 0,
				'aria-label': localize('welcomeAriaLabel', "逻辑核心架构师 (LogicCore Architect) 入门指南。")
			});
	...
	private async openWalkthroughSelector() {
		const selection = await this.quickInputService.pick(this.gettingStartedCategories
			...
			})), { canPickMany: false, matchOnDescription: true, matchOnDetail: true, title: localize('pickWalkthroughs', "打开入门指南...") });
		if (selection) {
	...
	private async buildCategoriesSlide(preserveFocus?: boolean) {
		...
		const showOnStartupCheckbox = new Toggle({
			...
			title: localize('checkboxTitle', "选中此项后，将在启动时显示此页面。"),
			...defaultToggleStyles
		});
		...
		const recentlyOpenedList = this.recentlyOpenedList = new GettingStartedIndexList(
			{
				...
				limit: 5,
				empty: $('.empty-recent', {},
					localize('noRecents', "您没有最近打开的文件夹，"),
	...
	private buildGettingStartedWalkthroughsList(): GettingStartedIndexList<IResolvedWalkthrough> {
		...
		const gettingStartedList = this.gettingStartedList = new GettingStartedIndexList(
			{
				title: localize('walkthroughs', "入门指南"),
