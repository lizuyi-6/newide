...
async function deleteFiles(explorerService: IExplorerService, workingCopyFileService: IWorkingCopyFileService, dialogService: IDialogService, configurationService: IConfigurationService, filesConfigurationService: IFilesConfigurationService, elements: ExplorerItem[], useTrash: boolean, skipConfirm = false, ignoreIfNotExists = false): Promise<void> {
	...
	if (useTrash) {
		primaryButton = isWindows ? nls.localize('deleteButtonLabelRecycleBin', "移至回收站(&&M)") : nls.localize({ key: 'deleteButtonLabelTrash', comment: ['&& denotes a mnemonic'] }, "移至废纸篓(&&M)");
	} else {
	...
	if (dirtyWorkingCopies.size) {
		...
		if (distinctElements.length > 1) {
			message = nls.localize('dirtyMessageFilesDelete', "您正在删除包含未保存更改的文件。是否继续？");
		} else if (distinctElements[0].isDirectory) {
			if (dirtyWorkingCopies.size === 1) {
				message = nls.localize('dirtyMessageFolderOneDelete', "您正在删除文件夹 {0}，其中包含 1 个有未保存更改的文件。是否继续？", distinctElements[0].name);
			} else {
				message = nls.localize('dirtyMessageFolderDelete', "您正在删除文件夹 {0}，其中包含 {1} 个有未保存更改的文件。是否继续？", distinctElements[0].name, dirtyWorkingCopies.size);
			}
		} else {
			message = nls.localize('dirtyMessageFileDelete', "您正在删除 {0}，该文件包含未保存的更改。是否继续？", distinctElements[0].name);
		}
	...
	if (!skipConfirm) {
		...
		if (readonlyResources.length) {
			...
			if (readonlyResources.length > 1) {
				message = nls.localize('readonlyMessageFilesDelete', "您正在删除配置为只读的文件。是否继续？");
			} else if (readonlyResources[0].isDirectory) {
				message = nls.localize('readonlyMessageFolderOneDelete', "您正在删除文件夹 {0}，该文件夹配置为只读。是否继续？", distinctElements[0].name);
			} else {
				message = nls.localize('readonlyMessageFolderDelete', "您正在删除文件 {0}，该文件配置为只读。是否继续？", distinctElements[0].name);
			}

			const response = await dialogService.confirm({
				type: 'warning',
				message,
				detail: nls.localize('continueDetail', "如果您继续，只读保护将被覆盖。"),
	...
	const deleteDetail = distinctElements.some(e => e.isDirectory) ? nls.localize('irreversible', "此操作不可逆！") :
		distinctElements.length > 1 ? nls.localize('restorePlural', "您可以使用“撤销”命令恢复这些文件。") : nls.localize('restore', "您可以使用“撤销”命令恢复此文件。");
	...
	// Confirm for moving to trash
		...
		detail += detail ? '\n' : '';
		if (isWindows) {
			detail += distinctElements.length > 1 ? nls.localize('undoBinFiles', "您可以从回收站恢复这些文件。") : nls.localize('undoBin', "您可以从回收站恢复此文件。");
		} else {
			detail += distinctElements.length > 1 ? nls.localize('undoTrashFiles', "您可以从废纸篓恢复这些文件。") : nls.localize('undoTrash', "您可以从废纸篓恢复此文件。");
		}
	...
	try {
		...
		let primaryButton: string;
		if (useTrash) {
			errorMessage = isWindows ? nls.localize('binFailed', "使用回收站删除失败。是否要改为永久删除？") : nls.localize('trashFailed', "使用废纸篓删除失败。是否要改为永久删除？");
...
async function askForOverwrite(fileService: IFileService, dialogService: IDialogService, targetResource: URI): Promise<boolean> {
	...
	const { confirmed } = await dialogService.confirm({
		...
		message: nls.localize('confirmOverwrite', "目标文件夹中已存在名为“{0}”的文件或文件夹。是否要替换它？", basename(targetResource.path)),
...
export class GlobalCompareResourcesAction extends Action2 {
	...
	constructor() {
		super({
			...
			metadata: {
				description: nls.localize2('compareFileWithMeta', "打开选择器以选择要与活动编辑器进行比较的文件。")
			}
		});
	}
...
export class SaveAllInGroupAction extends BaseSaveAllAction {
	...
	static readonly LABEL = nls.localize('saveAllInGroup', "保存组中所有文件");
...
export class FocusFilesExplorer extends Action2 {
	...
	static readonly LABEL = nls.localize2('focusFilesExplorer', "聚焦于文件资源管理器");
...
export class ShowActiveFileInExplorer extends Action2 {
	...
	static readonly LABEL = nls.localize2('showInExplorer', "在资源管理器中显示活动文件");
...
export class OpenActiveFileInEmptyWorkspace extends Action2 {
	...
	static readonly LABEL = nls.localize2('openFileInEmptyWorkspace', "在新空工作区中打开活动编辑器");
...
export function validateFileName(pathService: IPathService, item: ExplorerItem, name: string, os: OperatingSystem): { content: string; severity: Severity } | null {
	...
	if (names.some(folderName => !pathService.hasValidBasename(item.resource, os, folderName))) {
		...
		return {
			content: nls.localize('invalidFileNameError', "名称 **{0}** 不是有效的文件或文件夹名称。请选择其他名称。", trimLongName(escapedName)),
			severity: Severity.Error
		};
	}
	...
	if (names.some(name => /^\s|\s$/.test(name))) {
		return {
			content: nls.localize('fileNameWhitespaceWarning', "检测到文件或文件夹名称中包含前导或尾随空格。"),
