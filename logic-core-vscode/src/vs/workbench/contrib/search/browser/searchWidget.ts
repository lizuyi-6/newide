...
export class SearchWidget extends Widget {
	...
	private static readonly REPLACE_ALL_DISABLED_LABEL = nls.localize('search.action.replaceAll.disabled.label', "全部替换 (提交搜索以启用)");
	private static readonly REPLACE_ALL_ENABLED_LABEL = (keyBindingService2: IKeybindingService): string => {
		return keyBindingService2.appendKeybinding(nls.localize('search.action.replaceAll.enabled.label', "全部替换"), ReplaceAllAction.ID);
	};
