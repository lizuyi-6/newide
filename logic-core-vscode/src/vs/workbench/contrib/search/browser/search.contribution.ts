...
quickAccessRegistry.registerQuickAccessProvider({
	...
	placeholder: nls.localize('anythingQuickAccessPlaceholder', "按名称搜索文件（追加 {0} 跳转到行或 {1} 跳转到符号）", AbstractGotoLineQuickAccessProvider.GO_TO_LINE_PREFIX, GotoSymbolQuickAccessProvider.PREFIX),
...
quickAccessRegistry.registerQuickAccessProvider({
	...
	placeholder: nls.localize('symbolsQuickAccessPlaceholder', "输入要打开的符号名称。"),
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		[SEARCH_EXCLUDE_CONFIG]: {
			type: 'object',
			markdownDescription: nls.localize('exclude', "配置用于在全文搜索和快速打开文件搜索中排除文件和文件夹的 [glob 模式](https://code.visualstudio.com/docs/editor/codebasics#_advanced-search-options)。要从快速打开的最近打开列表中排除文件，模式必须是绝对的（例如 `**/node_modules/**`）。继承 `#files.exclude#` 设置中的所有 glob 模式。"),
			default: { '**/node_modules': true, '**/bower_components': true, '**/*.code-search': true },
			additionalProperties: {
				anyOf: [
					{
						type: 'boolean',
						description: nls.localize('exclude.boolean', "用于匹配文件路径的 glob 模式。设置为 true 或 false 以启用或禁用模式。"),
					},
					{
						type: 'object',
						properties: {
		...
		[SEARCH_MODE_CONFIG]: {
			...
			default: 'view',
			markdownDescription: nls.localize('search.mode', "控制新的“搜索：在文件中查找”和“在文件夹中查找”操作发生的位置：在搜索视图中，还是在搜索编辑器中。"),
			enumDescriptions: [
				nls.localize('search.mode.view', "在搜索视图中搜索，在面板或侧边栏中。"),
				nls.localize('search.mode.reuseEditor', "在现有的搜索编辑器中搜索（如果存在），否则在新的搜索编辑器中搜索。"),
		...
		'search.useRipgrep': {
			...
			deprecationMessage: nls.localize('useRipgrepDeprecated', "已弃用。考虑使用“search.usePCRE2”以获得高级正则表达式功能支持。"),
			default: true
		},
		'search.maintainFileSearchCache': {
			type: 'boolean',
			deprecationMessage: nls.localize('maintainFileSearchCacheDeprecated', "搜索缓存保存在从不关闭的扩展主机中，因此不再需要此设置。"),
			description: nls.localize('search.maintainFileSearchCache', "启用时，searchService 进程将保持活动状态，而不是在一小时不活动后关闭。这将使文件搜索缓存保留在内存中。"),
			default: false
		},
		...
		'search.useGlobalIgnoreFiles': {
			type: 'boolean',
			markdownDescription: nls.localize('useGlobalIgnoreFiles', "控制在搜索文件时是否使用全局 gitignore 文件（例如，来自 `$HOME/.config/git/ignore`）。需要启用 {0}。", '`#search.useIgnoreFiles#`'),
			default: false,
			scope: ConfigurationScope.RESOURCE
		},
		...
		'search.quickOpen.includeHistory': {
			type: 'boolean',
			description: nls.localize('search.quickOpen.includeHistory', "是否在快速打开的文件结果中包含最近打开文件的结果。"),
			default: true
		},
		'search.quickOpen.history.filterSortOrder': {
			...
			default: 'default',
			enumDescriptions: [
				nls.localize('filterSortOrder.default', '历史条目根据使用的过滤值按相关性排序。更相关的条目首先出现。'),
				nls.localize('filterSortOrder.recency', '历史条目按最近程度排序。最近打开的条目首先出现。')
			],
			description: nls.localize('filterSortOrder', "控制过滤时快速打开中编辑器历史记录的排序顺序。")
		},
		'search.followSymlinks': {
			type: 'boolean',
			description: nls.localize('search.followSymlinks', "控制是否在搜索时跟随符号链接。"),
			default: true
		},
		'search.smartCase': {
			type: 'boolean',
		...
		'search.location': {
			...
			enum: ['sidebar', 'panel'],
			default: 'sidebar',
			description: nls.localize('search.location', "控制搜索是显示为侧边栏中的视图还是显示在面板区域中作为面板，以获得更多水平空间。"),
			deprecationMessage: nls.localize('search.location.deprecationMessage', "此设置已弃用。您可以将搜索图标拖动到新位置。")
		},
		'search.maxResults': {
			type: ['number', 'null'],
			default: DEFAULT_MAX_SEARCH_RESULTS,
			markdownDescription: nls.localize('search.maxResults', "控制搜索结果的最大数量，可以将其设置为 `null`（空）以返回无限结果。")
		},
		'search.collapseResults': {
			type: 'string',
			enum: ['auto', 'alwaysCollapse', 'alwaysExpand'],
			enumDescriptions: [
				nls.localize('search.collapseResults.auto', "结果少于 10 个的文件将展开。其他文件将折叠。"),
				'',
				''
			],
			default: 'alwaysExpand',
			description: nls.localize('search.collapseAllResults', "控制是否折叠或展开搜索结果。"),
		},
		...
		'search.usePCRE2': {
			type: 'boolean',
			default: false,
			description: nls.localize('search.usePCRE2', "是否在文本搜索中使用 PCRE2 正则表达式引擎。这允许使用一些高级正则表达式功能，如前瞻和反向引用。但是，并非所有 PCRE2 功能都受支持 - 仅支持 JavaScript 也支持的功能。"),
		...
		'search.actionsPosition': {
			...
			enum: ['auto', 'right'],
			enumDescriptions: [
				nls.localize('search.actionsPositionAuto', "当搜索视图较窄时将操作栏放置在右侧，当搜索视图较宽时紧随内容之后。"),
		...
		'search.searchOnType': {
			type: 'boolean',
			default: true,
			description: nls.localize('search.searchOnType', "输入时搜索所有文件。")
		},
		'search.seedWithNearestWord': {
			type: 'boolean',
			default: false,
			description: nls.localize('search.seedWithNearestWord', "当活动编辑器没有选择时，启用从光标最近的单词开始搜索。")
		},
		'search.seedOnFocus': {
			type: 'boolean',
			default: false,
			markdownDescription: nls.localize('search.seedOnFocus', "聚焦搜索视图时，将搜索查询更新为编辑器选定的文本。这发生在单击或触发 `workbench.views.search.focus` 命令时。")
		},
		'search.searchOnTypeDebouncePeriod': {
			type: 'number',
			default: 300,
			markdownDescription: nls.localize('search.searchOnTypeDebouncePeriod', "当启用 {0} 时，控制键入字符与搜索开始之间的超时（以毫秒为单位）。当禁用 {0} 时无效。", '`#search.searchOnType#`')
		},
		'search.searchEditor.doubleClickBehaviour': {
			...
			enumDescriptions: [
				...
				nls.localize('search.searchEditor.doubleClickBehaviour.openLocationToSide', "双击在侧边编辑器组中打开结果（如果尚不存在则创建一个）。"),
			],
		...
		'search.searchEditor.singleClickBehaviour': {
			...
			enumDescriptions: [
				...
				nls.localize('search.searchEditor.singleClickBehaviour.peekDefinition', "单击打开快速预览定义窗口。"),
			],
		...
		'search.searchEditor.reusePriorSearchConfiguration': {
			type: 'boolean',
			default: false,
			markdownDescription: nls.localize({ key: 'search.searchEditor.reusePriorSearchConfiguration', comment: ['"Search Editor" is a type of editor that can display search results. "includes, excludes, and flags" refers to the "files to include" and "files to exclude" input boxes, and the flags that control whether a query is case-sensitive or a regex.'] }, "启用时，新的搜索编辑器将重用先前打开的搜索编辑器的包含、排除和标志。")
		},
		'search.searchEditor.defaultNumberOfContextLines': {
			type: ['number', 'null'],
			default: 1,
			markdownDescription: nls.localize('search.searchEditor.defaultNumberOfContextLines', "创建新搜索编辑器时使用的默认周围上下文行数。如果使用 `#search.searchEditor.reusePriorSearchConfiguration#`，则可以将其设置为 `null`（空）以使用先前搜索编辑器的配置。")
		},
		'search.searchEditor.focusResultsOnSearch': {
			type: 'boolean',
			default: false,
			markdownDescription: nls.localize('search.searchEditor.focusResultsOnSearch', "触发搜索时，聚焦搜索编辑器结果而不是搜索编辑器输入。")
		},
		'search.sortOrder': {
			type: 'string',
			...
			enumDescriptions: [
				nls.localize('searchSortOrder.default', "结果按文件夹和文件名称按字母顺序排序。"),
				nls.localize('searchSortOrder.filesOnly', "结果按文件名称按字母顺序排序，忽略文件夹顺序。"),
				nls.localize('searchSortOrder.type', "结果按文件扩展名按字母顺序排序。"),
				nls.localize('searchSortOrder.modified', "结果按文件最后修改日期降序排序。"),
				nls.localize('searchSortOrder.countDescending', "结果按每个文件的数量降序排序。"),
				nls.localize('searchSortOrder.countAscending', "结果按每个文件的数量升序排序。")
			],
		...
		'search.defaultViewMode': {
			...
			enumDescriptions: [
				nls.localize('scm.defaultViewMode.tree', "以树状显示搜索结果。"),
				nls.localize('scm.defaultViewMode.list', "以列表状显示搜索结果。")
			],
			description: nls.localize('search.defaultViewMode', "控制默认搜索结果视图模式。")
		},
		'search.quickAccess.preserveInput': {
			type: 'boolean',
			description: nls.localize('search.quickAccess.preserveInput', "控制下次打开时是否恢复上次键入的快速搜索输入。"),
			default: false
		},
		'search.experimental.closedNotebookRichContentResults': {
			type: 'boolean',
			description: nls.localize('search.experimental.closedNotebookResults', "为关闭的笔记本显示笔记本编辑器富内容结果。更改此设置后，请刷新您的搜索结果。"),
			default: false
		},
		'search.searchView.semanticSearchBehavior': {
			...
			enumDescriptions: [
				...
				nls.localize('search.searchView.semanticSearchBehavior.auto', "每次搜索都自动请求语义结果。")
			],
			tags: ['preview'],
		},
