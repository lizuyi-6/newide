...
export class WelcomeView extends ViewPane {
	...
	static readonly LABEL: ILocalizedString = localize2('run', "运行");
...
viewsRegistry.registerViewWelcomeContent(WelcomeView.ID, {
	content: localize(
		...
		"[打开文件](command:{0}) 以进行调试或运行。", OpenFileAction.ID
	),
...
viewsRegistry.registerViewWelcomeContent(WelcomeView.ID, {
	content: `[${localize('runAndDebugAction', "运行和调试")}${debugKeybindingLabel}](command:${DEBUG_START_COMMAND_ID})`,
...
viewsRegistry.registerViewWelcomeContent(WelcomeView.ID, {
	content: localize({ key: 'customizeRunAndDebug2', comment: ['{Locked="launch.json"}', '{Locked="["}', '{Locked="]({0})"}'] },
		"要自定义运行和调试，请[创建 launch.json 文件]({0})。", `${createCommandUri(DEBUG_CONFIGURE_COMMAND_ID, { addNew: true }).toString()}`),
...
viewsRegistry.registerViewWelcomeContent(WelcomeView.ID, {
	content: localize(
		...
		"要自定义运行和调试，请[打开文件夹]({0})并创建 launch.json 文件。", createCommandUri(OpenFolderAction.ID).toString()),
	when: ContextKeyExpr.and(CONTEXT_DEBUGGERS_AVAILABLE, WorkbenchStateContext.isEqualTo('empty')),
	group: ViewContentGroups.Debug
});

viewsRegistry.registerViewWelcomeContent(WelcomeView.ID, {
	content: localize('allDebuggersDisabled', "所有调试扩展已禁用。请启用调试扩展或从市场安装一个新的。"),
