...
abstract class BaseOpenRecentAction extends Action2 {
	...
	private readonly removeFromRecentlyOpened: IQuickInputButton = {
		...
		tooltip: localize('remove', "从最近打开中移除")
	};
	...
	private readonly dirtyRecentlyOpenedFolder: IQuickInputButton = {
		...
		tooltip: localize('dirtyRecentlyOpenedFolder', "包含未保存文件的文件夹"),
		alwaysVisible: true
	};

	private readonly dirtyRecentlyOpenedWorkspace: IQuickInputButton = {
		...this.dirtyRecentlyOpenedFolder,
		tooltip: localize('dirtyRecentlyOpenedWorkspace', "包含未保存文件的工作区"),
	};
	...
	private readonly windowOpenedRecentlyOpenedFolder: IQuickInputButton = {
		...
		tooltip: localize('openedRecentlyOpenedFolder', "在窗口中打开的文件夹"),
		alwaysVisible: true
	};

	private readonly windowOpenedRecentlyOpenedWorkspace: IQuickInputButton = {
		...this.windowOpenedRecentlyOpenedFolder,
		tooltip: localize('openedRecentlyOpenedWorkspace', "在窗口中打开的工作区"),
	};
	...
	private readonly activeWindowOpenedRecentlyOpenedFolder: IQuickInputButton = {
		...
		tooltip: localize('activeOpenedRecentlyOpenedFolder', "在活动窗口中打开的文件夹"),
		alwaysVisible: true
	};

	private readonly activeWindowOpenedRecentlyOpenedWorkspace: IQuickInputButton = {
		...this.activeWindowOpenedRecentlyOpenedFolder,
		tooltip: localize('activeOpenedRecentlyOpenedWorkspace', "在活动窗口中打开的工作区"),
	};
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const pick = await quickInputService.pick(picks, {
			...
			placeHolder: isMacintosh ? localize('openRecentPlaceholderMac', "选择以打开 (按住 Cmd 键强制在新窗口打开，或按住 Option 键在当前窗口打开)") : localize('openRecentPlaceholder', "选择以打开 (按住 Ctrl 键强制在新窗口打开，或按住 Alt 键在当前窗口打开)"),
			...
			onDidTriggerItemButton: async context => {
				...
				// Dirty Folder/Workspace
					...
					const { confirmed } = await dialogService.confirm({
						title: isDirtyWorkspace ? localize('dirtyWorkspace', "包含未保存文件的工作区") : localize('dirtyFolder', "包含未保存文件的文件夹"),
						message: isDirtyWorkspace ? localize('dirtyWorkspaceConfirm', "是否要打开工作区以查看未保存的文件？") : localize('dirtyFolderConfirm', "是否要打开文件夹以查看未保存的文件？"),
						detail: isDirtyWorkspace ? localize('dirtyWorkspaceConfirmDetail', "包含未保存文件的工作区无法删除，除非所有未保存的文件已被保存或还原。") : localize('dirtyFolderConfirmDetail', "包含未保存文件的文件夹无法删除，除非所有未保存的文件已被保存或还原。")
					});
	...
	private toQuickPick(modelService: IModelService, languageService: ILanguageService, labelService: ILabelService, recent: IRecent, kind: { isDirty: boolean; windowState?: { isActive: boolean } }): IRecentlyOpenedPick {
		...
		return {
			iconClasses,
			label: name,
			ariaLabel: kind.isDirty ? isWorkspace ? localize('recentDirtyWorkspaceAriaLabel', "{0}, 包含未保存更改的工作区", name) : localize('recentDirtyFolderAriaLabel', "{0}, 包含未保存更改的文件夹", name) : name,
...
class ToggleFullScreenAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: {
				...localize2('toggleFullScreen', "切换全屏"),
				mnemonicTitle: localize({ key: 'miToggleFullScreen', comment: ['&& denotes a mnemonic'] }, "全屏(&&F)"),
			},
...
class NewWindowAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: {
				...localize2('newWindow', "新窗口"),
				mnemonicTitle: localize({ key: 'miNewWindow', comment: ['&& denotes a mnemonic'] }, "新窗口(&&W)"),
			},
			f1: true,
			keybinding: {
				weight: KeybindingWeight.WorkbenchContrib,
...
class BlurAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('blur', '从聚焦元素移除键盘焦点')
		});
	}
