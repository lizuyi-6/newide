...
export const ADD_ROOT_FOLDER_LABEL: ILocalizedString = localize2('addFolderToWorkspace', '将文件夹添加到工作区...');
...
async function selectWorkspaceFolders(accessor: ServicesAccessor): Promise<URI[] | undefined> {
	...
	const folders = await dialogsService.showOpenDialog({
		openLabel: mnemonicButtonLabel(localize({ key: 'add', comment: ['&& denotes a mnemonic'] }, "添加(&&A)")),
		title: localize('addFolderToWorkspaceTitle', "将文件夹添加到工作区"),
...
CommandsRegistry.registerCommand(PICK_WORKSPACE_FOLDER_COMMAND_ID, async function (accessor, args?: [IPickOptions<IQuickPickItem>, CancellationToken]) {
	...
	if (!options.placeHolder) {
		options.placeHolder = localize('workspaceFolderPickerPlaceholder', "选择工作区文件夹");
	}
