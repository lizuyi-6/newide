...
export function createTextInputActions(clipboardService: IClipboardService, logService: ILogService): IAction[] {
	return [
		...
		toAction({ id: 'undo', label: localize('undo', "撤销"), run: () => getActiveDocument().execCommand('undo') }),
		toAction({ id: 'redo', label: localize('redo', "重做"), run: () => getActiveDocument().execCommand('redo') }),
		new Separator(),
		toAction({
			id: 'editor.action.clipboardCutAction', label: localize('cut', "剪切"), run: () => {
				logService.trace('TextInputActionsProvider#cut');
				getActiveDocument().execCommand('cut');
			}
		}),
		toAction({
			id: 'editor.action.clipboardCopyAction', label: localize('copy', "复制"), run: () => {
		...
		new Separator(),
		toAction({ id: 'editor.action.clipboardselectAll', label: localize('selectAll', "全选"), run: () => getActiveDocument().execCommand('selectAll') })
	];
}
